...
import {
  getFirestore, collection, doc, getDoc, getDocs, setDoc, addDoc, updateDoc, deleteDoc, query, where, serverTimestamp, onSnapshot
...
const els = {
  ...
  bottomNav: el('bottomNav'),
  
  profileAvatar: el('profileAvatar'), profileUsername: el('profileUsername'), profileEmail: el('profileEmail'),
  profileBalance: el('profileBalance'), profileRole: el('profileRole'), profileUid: el('profileUid'), profileLogoutBtn: el('profileLogoutBtn'),
...
window.showToast = function(message, type = 'success') {
  if (!els.toast) return;
  els.toast.textContent = message; els.toast.className = `toast ${type}`; els.toast.classList.remove('hidden');
...
function formatDate(value) {
  ...
  return new Date(value?.seconds ? value.seconds * 1000 : value).toLocaleString('vi-VN');
}
...
function showSuccessModal(title, bodyHTML) {
  ...
  const svgContainer = els.successModal.querySelector('.success-animation');
  if (svgContainer) { const newSvg = svgContainer.innerHTML; svgContainer.innerHTML = newSvg; }
  els.successModalTitle.textContent = title;
  els.successModalBody.innerHTML = bodyHTML;
...
async function processAdminCommand(rawCmd) {
  ...
  try {
    switch (cmd) {
      ...
      case '/setbal': if (args.length < 2) throw new Error('Cú pháp: /setbal [UID] [Số tiền]'); await updateDoc(doc(db, 'users', args[0]), { balance: Number(args[1]) || 0 }); log(`Set số dư ${args[0]} thành ${formatMoney(args[1])}`); break;
      case '/addbal': if (args.length < 2) throw new Error('Cú pháp: /addbal [UID] [Số tiền]'); const addAmt = Number(args[1]); const uRef = await getDoc(doc(db, 'users', args[0])); await updateDoc(doc(db, 'users', args[0]), { balance: (uRef.data().balance || 0) + addAmt }); log(`Cộng/trừ ${formatMoney(addAmt)} cho ${args[0]}`); break;
      case '/giveall': if (!args[0]) throw new Error('Cú pháp: /giveall [Số tiền]'); const amount = Number(args[0]); const allUsers = await getDocs(collection(db, 'users')); await Promise.all(allUsers.docs.map(u => updateDoc(u.ref, { balance: (u.data().balance || 0) + amount }))); log(`Tặng ${formatMoney(amount)} cho toàn Server.`); break;
      case '/ban': if (!args[0]) throw new Error('Cú pháp: /ban [UID] [Lý do]'); await updateDoc(doc(db, 'users', args[0]), { banned: true, banReason: args.slice(1).join(' ') || 'Vi phạm' }); log(`Đã BAN ${args[0]}`); break;
      case '/unban': if (!args[0]) throw new Error('Cú pháp: /unban [UID]'); await updateDoc(doc(db, 'users', args[0]), { banned: false, banReason: '' }); log(`Đã UNBAN ${args[0]}`); break;
      case '/role': if (args.length < 2) throw new Error('Cú pháp: /role [UID] [admin/user]'); await updateDoc(doc(db, 'users', args[0]), { role: args[1] }); log(`Set Role ${args[1]} cho ${args[0]}`); break;
      ...
      default: log('Lệnh không tồn tại. Gõ /help', true);
    }
    await loadUsers();
...
function renderProducts() {
  ...
  els.shopContainer.querySelectorAll('[data-buy]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        const pRef = doc(db, 'products', btn.dataset.buy); const pSnap = await getDoc(pRef);
        ...
        btn.textContent = 'HẾT HÀNG'; btn.classList.remove('btn-glow'); btn.classList.add('btn-disabled'); btn.disabled = true;
        const card = btn.closest('.product-card'); const badge = card.querySelector('.status-badge'); badge.textContent = 'Đã Bán'; badge.className = 'status-badge sold';
        ...
        await refreshAll();
      } catch (e) { window.showToast(e.message, 'error'); } finally { setLoading(btn, false); }
    });
  });
}
...
function renderAdminProducts() {
  ...
  els.adminProductsTable.querySelectorAll('[data-edit]').forEach(b => b.addEventListener('click', () => {
    ...
    state.editProductId = p.id; els.pTitle.value = p.title || ''; els.pGame.value = p.game || ''; els.pDescription.value = p.description || '';
